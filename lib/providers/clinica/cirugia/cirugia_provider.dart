...
class CirugiaProvider extends ChangeNotifier {
  ...
  int _selectedSquareCirugia = 0;
  int get selectedSquareCirugia => _selectedSquareCirugia;

  void setSelectSquareCirugia(int square) {
    _selectedSquareCirugia = square;
    notifyListeners();
  }
  ...
  bool _loadingDatosCirugia = false;
  bool get loadingDatosCirugia => _loadingDatosCirugia;

  setLoadingDatosCirugia(bool value) {
    _loadingDatosCirugia = value;
    notifyListeners();
  }

  //cambio a true si el POST se realizo de manera correcta
  bool _OKpostDatosCirugia = false;
  bool get OkpostDatosCirugia => _OKpostDatosCirugia;

  setOKsendDatosCirugia(bool value) {
    _OKpostDatosCirugia = value;
    notifyListeners();
  }
  ...
  //DATOS FORM DATOS CIRUGIA

  //archivo de autorizacion quirurgica
  String _fileAutorizacionQuirurgica = '';

  String get fileAutorizacionQuirurgica => _fileAutorizacionQuirurgica;

  void setAutorizacionQuirurgica(String value) {
    _fileAutorizacionQuirurgica = value;
    notifyListeners();
  }

  //Cargando la imagen del pre cirugio
  XFile? imageCirugia;
  File? lastImageCirugia;

  Future addPhotoCirugia() async {
    imageCirugia = await ImagePicker().pickImage(source: ImageSource.camera);
    if (imageCirugia != null) {
      lastImageCirugia = File(imageCirugia!.path);
      _imagesCirugia.add(File(imageCirugia!.path));
      notifyListeners();
    }
  }

  void removePhotoCirugia(int index) {
    _imagesCirugia.removeAt(index);
    notifyListeners();
  }

  List<File> _imagesCirugia = [];

  List<File> get imagesCirugia => _imagesCirugia;

  // Future<void> addPhotoCirugia() async {
  //   XFile? image = await ImagePicker().pickImage(source: ImageSource.camera);
  //   if (image != null) {
  //     _imagesCirugia.add(File(image.path));
  //     notifyListeners();
  //   }
  // }
  Future getFromUrlCirugia(String url) async {
    final response = await http.get(Uri.parse('$_urlBaseImagen$url'));
    if (response.statusCode == 200) {
      final bytes = response.bodyBytes;
      final directory = await getApplicationDocumentsDirectory();
      ...
      final file = File(imagePath);
      await file.writeAsBytes(bytes);
      lastImageCirugia = file;
      notifyListeners();
    }
  }

  List<TextEditingController> controllersTratamientoCirugia = [];

  void agregarTratamiento() {
    controllersTratamientoCirugia.add(TextEditingController());
    for (var controller in controllersTratamientoCirugia) {
  ...
  Future<void> getEncargadosCirugia() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  void saveSignature(SignatureController controller, BuildContext context,
      CirugiaProvider signatureProvider) async {
  ...
  Future<bool> enviarDatos(
    ...
    String tipoCirugiaf,
    String archivoAuthQuirurgicaf,
    String anestesicosf,
    String obsPreCirugiaf,
    String obsPostOperatoriof,
    List<TextEditingController> tratamientosListaf,
    String recomendacionCirugiaf,
    ...
    String urlFinal = '${_urlBase}clinica/ficha-cirugia';

    setLoadingDatosCirugia(true);
    ...
    try {
      ...
      //EJEMPLO
      /*  request.fields['ficha_clinica_id'] = '1';
    request.fields['muestrasRequeridas'] = 'tamanoMascotaF';
    request.fields['pruebasRequeridas'] = 'propietario_id';

    for (final fileName in fileHemogramaf) {
      final file = File(fileName);

      request.files.add(
        await http.MultipartFile.fromPath('hemograma[]', file.path),
      );
    } */
      //EJEMPLO HASTA AQUIIII
      ...
      request.fields['cirugia[tipoCirugia]'] = tipoCirugiaf;

      request.files.add(await http.MultipartFile.fromPath(
          'archivo[autorizacionQuirurgica]', archivoAuthQuirurgicaf));

      request.fields['cirugia[anestesicos]'] = anestesicosf;
      request.fields['cirugia[observacionesPrecirugia]'] = obsPreCirugiaf;
      request.fields['cirugia[postOperatorio]'] = obsPostOperatoriof;

      for (var image in imagesCirugia) {
        var tempFilePathCirugiaAvance =
            '${(await getTemporaryDirectory()).path}/${DateTime.now().millisecondsSinceEpoch}.jpg';
        await image.copy(tempFilePathCirugiaAvance);

        var imageUploadFotoCirugiaPost = await http.MultipartFile.fromPath(
          'archivo[avancesPostOperatorios][]',
          tempFilePathCirugiaAvance,
        );
        request.files.add(imageUploadFotoCirugiaPost);
      }

      for (int i = 0; i < tratamientosListaf.length; i++) {
        request.fields['cirugia[tratamiento][$i]'] = tratamientosListaf[i].text;
      }

      request.fields['cirugia[recomendaciones]'] = recomendacionCirugiaf;
      ...
      if (responseData['code'] == 200 &&
        ...
        setLoadingDatosCirugia(false);
        setOKsendDatosCirugia(true);
        notifyListeners();
        return OkpostDatosCirugia;
      } else {
        print('Error en la solicitud cirugia: ${responseData}');
        setLoadingDatosCirugia(false);
        BotToast.showText(
            text: responseData['data'], duration: const Duration(seconds: 3));
        setOKsendDatosCirugia(false);
        notifyListeners();
        return OkpostDatosCirugia;
      }
    } catch (e) {
      print('errooorrr cirugia $e');
      setLoadingDatosCirugia(false);
      BotToast.showText(
  ...
  Future<bool> enviarDatosAntiguo(
    ...
    String urlFinal = '${_urlBase}clinica/ficha-cirugia/$idPacienteAntiguo';
  ...
  Future<bool> actualizarDatos(
    ...
    String obsPreCirugiaf,
    String obsPostOperatoriof,
    File? fotoAvancePostOperatoriof,
    List<TextEditingController> tratamientosListaf,
    String recomendacionCirugiaf,
    ...
    String urlFinal = '${_urlBase}update/ficha-cirugia/${_fichaId}';
    ...
    try {
      ...
      print('error en archivo autorizacion quirurgica $e');
    }
    ...
    var tempDirCirugiaAvance = Directory.systemTemp;
    var tempFilePathCirugiaAvance =
        '${tempDirCirugiaAvance.path}/${DateTime.now().millisecondsSinceEpoch}.jpg';

    if (fotoAvancePostOperatoriof != null) {
      await fotoAvancePostOperatoriof.copy(tempFilePathCirugiaAvance);

      var imageUploadFotoCirugiaPost = await http.MultipartFile.fromPath(
          'archivo[avancesPostOperatorios]', tempFilePathCirugiaAvance);
